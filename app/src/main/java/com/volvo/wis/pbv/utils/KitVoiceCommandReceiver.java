...
public class KitVoiceCommandReceiver extends BaseVoiceCommandReceiver {

    private KitActivity kitActivity;

    public KitVoiceCommandReceiver(KitActivity activity) {
        LOG_TAG = activity.LOG_TAG;
        kitActivity = activity;
        kitActivity.registerReceiver(this, new IntentFilter(VuzixSpeechClient.ACTION_VOICE_COMMAND));
        ...
        try {
            ...
            // See what we've done
            Log.i(kitActivity.LOG_TAG, sc.dump());

            // The recognizer may not yet be enabled in Settings. We can enable this directly
            VuzixSpeechClient.EnableRecognizer(kitActivity, true);
            ...
            // SDK
            Log.e(kitActivity.LOG_TAG, "Voice SDK only runs on Vuzix Smart Glasses w. Vuzix Speech SDK support");
            Log.e(kitActivity.LOG_TAG, e.getMessage());
            e.printStackTrace();
            activity.finish();
        } catch (Exception e) {
            Log.e(kitActivity.LOG_TAG, "Error setting custom vocabulary: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void unregister() {
        try {
            kitActivity.unregisterReceiver(this);
            Log.i(kitActivity.LOG_TAG, "Custom vocab removed");
            kitActivity = null;
        } catch (Exception e) {
            Log.e(kitActivity.LOG_TAG, "Custom vocab died " + e.getMessage());
        }
    }
}
