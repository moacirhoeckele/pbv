...
public class LoginVoiceCommandReceiver extends BaseVoiceCommandReceiver {
    ...
    public LoginVoiceCommandReceiver(LoginActivity activity) {
        ...
        loginActivity.registerReceiver(this, new IntentFilter(VuzixSpeechClient.ACTION_VOICE_COMMAND));
        ...
        try {
            ...
            // See what we've done
            Log.i(loginActivity.LOG_TAG, sc.dump());

            // The recognizer may not yet be enabled in Settings. We can enable this directly
            VuzixSpeechClient.EnableRecognizer(loginActivity, true);
            ...
            // SDK
            Log.e(loginActivity.LOG_TAG, "Voice SDK only runs on Vuzix Smart Glasses w. Vuzix Speech SDK support");
            Log.e(loginActivity.LOG_TAG, e.getMessage());
            e.printStackTrace();
            activity.finish();
        } catch (Exception e) {
            Log.e(loginActivity.LOG_TAG, "Error setting custom vocabulary: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void unregister() {
        try {
            loginActivity.unregisterReceiver(this);
            Log.i(loginActivity.LOG_TAG, "Custom vocab removed");
            loginActivity = null;
        } catch (Exception e) {
            Log.e(loginActivity.LOG_TAG, "Custom vocab died " + e.getMessage());
        }
    }
}
